        ...
        <AnimatePresence initial={false}>
          {emotionCards.map((card) => (
            <motion.div
              ...
              className={`p-4 rounded-lg border-2 ${card.color || getSentimentColor(card.sentiment)} transition-all duration-200 hover:shadow-md`}
            >
              ...
              {card.keywords.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {card.keywords.slice(0, 5).map((keyword, index) => (
                    ...
                    >
                      #{typeof keyword === 'string' ? keyword : keyword.key} {typeof keyword === 'object' ? `(${keyword.count})` : ''}
                    </span>
                  ))}
